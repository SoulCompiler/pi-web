...
服务就绪后，命令行会尝试自动打开浏览器。如果没有打开，请访问 [http://127.0.0.1:16666](http://127.0.0.1:16666)。Pi Web 默认仅监听 `127.0.0.1`。
...
| --- | --- | --- |
| `--help`、`-h` | 打印启动选项并退出 | — |
| `--port <端口>`、`-p <端口>` 或 `PORT` | 服务端口 | `16666` |
...
开发服务器运行在 [http://127.0.0.1:16666](http://127.0.0.1:16666)。常用检查命令：
