...
Когда сервер будет готов, CLI попытается открыть браузер автоматически. Если этого не произошло, откройте [http://127.0.0.1:16666](http://127.0.0.1:16666). По умолчанию Pi Web прослушивает только `127.0.0.1`.
...
| `--port <port>`, `-p <port>` или `PORT` | Порт сервера | `16666` |
...
Сервер разработки доступен по адресу [http://127.0.0.1:16666](http://127.0.0.1:16666). Основные проверки запускаются командами:
