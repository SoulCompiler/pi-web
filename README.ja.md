...
サーバーの準備が整うと、CLI はブラウザーを自動的に開こうとします。開かない場合は [http://127.0.0.1:16666](http://127.0.0.1:16666) にアクセスしてください。Pi Web はデフォルトで `127.0.0.1` のみをリッスンします。
...
| `--port <port>`、`-p <port>`、または `PORT` | サーバーポート | `16666` |
...
開発サーバーは [http://127.0.0.1:16666](http://127.0.0.1:16666) で動作します。一般的なチェックは次のコマンドで実行します：
